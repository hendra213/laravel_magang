            ...
            <div class="mx-auto max-w-screen-xl">
                    ...
                    @foreach ($produks as $produk)
                        <div
                            ...
                            <div class="p-5">
                                <h5 class="mb-2 text-xl font-bold text-gray-900 dark:text-white">{{ $produk->nama }}</h5>
                                <p class="text-sm text-gray-700 dark:text-gray-300 line-clamp-2">{{ $produk->deskripsi }}
                                </p>
                                <button data-modal-target="modal-detail-{{ $produk->id }}"
                                    data-modal-toggle="modal-detail-{{ $produk->id }}"
                                    class="mt-4 inline-flex items-center px-3 py-2 text-sm font-medium text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:outline-none">
                                    Lihat Detail
                                </button>
                            </div>
                        </div>
                        ...
                        <div id="modal-detail-{{ $produk->id }}" tabindex="-1" aria-hidden="true"
                            class="hidden fixed top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal h-full bg-black bg-opacity-50">
